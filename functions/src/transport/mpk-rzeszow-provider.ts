...
async function fetchMpkTripSchedule(tripId: unknown, delaySeconds: number): Promise<MpkTripSchedule> {
  ...
  const [tripDetails, { value: stopPointIndex }] = await Promise.all([
    fetchJsonWithRetry<{ stops?: any[]; shape?: unknown }>(`${TRIP_STOPS_URL}?${params.toString()}`, { headers: REQUEST_HEADERS }).catch(() => ({ stops: [], shape: [] })),
    loadStopPointIndex(),
  ]);
  const stops = Array.isArray(tripDetails.stops) ? tripDetails.stops : [];
  const routeShape = normalizeRouteShape(tripDetails.shape);
  if (!Array.isArray(stops) || stops.length === 0) return { schedule: [], routeStops: [], routePath: [], routeShape };
