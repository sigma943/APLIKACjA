...
exports.pkpIntercityProvider = void 0;
...
const PORTAL_GPS_REQUEST_BBOX = [49.6, 15.0, 54.2, 24.9];
...
async function fetchJson(path, params, retries = 2) {
    let lastError;
    for (let attempt = 0; attempt <= retries; attempt += 1) {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`PKP Intercity API HTTP ${response.status}: ${text.slice(0, 240)}`);
...
function readString(source, paths, fallback = '') {
...
function readDateMs(source, paths) {
    const value = readFirst(source, paths);
...
function numericId(raw, fallbackSeed) {
...
function normalizeTrainCategory(raw) {
    const category = raw.toUpperCase().replace(/[^A-Z]/g, '');
    if (category.includes('EIP'))
        return 'EIP';
    if (category.includes('EIC'))
        return 'EIC';
    if (category.includes('IC'))
        return 'IC';
    if (category.includes('TLK'))
        return 'TLK';
    return category || 'IC';
}
function getIconVariant(category) {
    return ICON_VARIANTS.has(category) ? category : 'IC';
}
function normalizeIso(ms) {
    return Number.isFinite(ms) ? new Date(ms).toISOString() : null;
}
function getRouteStops(operation) {
...
function stationNameFromRaw(stop) {
...
function stationIdFromRaw(stop) {
...
function routeStopTimeMs(stop, type) {
...
function getStopDelayMinutes(stop, plannedMs, realMs) {
    const explicit = readNumber(stop, ['delayMinutes', 'delay', 'arrivalDelayMinutes', 'departureDelayMinutes']);
    if (Number.isFinite(explicit))
        return explicit;
    if (Number.isFinite(plannedMs) && Number.isFinite(realMs))
        return Math.round((realMs - plannedMs) / 60000);
    return 0;
}
function buildScheduleStops(rawStops, stations, now) {
    const routeStops = [];
    ...
    rawStops.forEach((stop, index) => {
        ...
        const lookup = stations[String(stationIdRaw || '')] ||
        ...
        const timeType = readFirst(stop, ['plannedDeparture', 'actualDeparture', 'departureDelayMinutes'])
            ? 'departure'
            : 'arrival';
        const effectiveRealMs = Number.isFinite(realMs)
            ? realMs
            : Number.isFinite(plannedMs)
                ? plannedMs + delayMinutes * 60000
                : Number.NaN;
        ...
        routeStops.push({
            ...
            isPast: Number.isFinite(effectiveRealMs) ? effectiveRealMs < now - 2 * 60000 : false,
...
function getTrainDelaySeconds(operation, stops, now) {
    const explicitMinutes = readNumber(operation, ['delayMinutes', 'delay', 'currentDelayMinutes', 'maxDelayMinutes']);
    if (Number.isFinite(explicitMinutes) && Math.abs(explicitMinutes) <= 300)
    ...
    const upcoming = stops.find((stop) => {
        ...
        return !stop.isPast && Number.isFinite(timeMs) && timeMs >= now - 2 * 60000;
    });
    if (!upcoming?.planned || !upcoming.real)
        return 0;
...
function getDataAgeSec(operation, now) {
...
function relationFromOperation(operation, routeStops) {
...
function canonicalTrainNumber(raw) {
...
function operationTrainNumber(operation) {
    ...
    if (scheduleId && direct === scheduleId)
        return '';
    if (/^20\d{2}$/.test(direct) && direct === scheduleId)
        return '';
    return direct;
}
...
function isValidPortalGpsTrain(value) {
    return isRecord(value);
}
function parsePortalGpsPoints(payload) {
    ...
    for (const train of trains) {
        ...
        if (!Number.isFinite(lat) || !Number.isFinite(lng))
            continue;
        if (lat < 48 || lat > 55.5 || lng < 13.5 || lng > 25.5)
            continue;
...
async function fetchPortalGpsSnapshot(bbox) {
    void bbox;
    const bounds = PORTAL_GPS_REQUEST_BBOX;
    const cacheKey = buildGpsCacheKey();
    return (0, cache_1.getCachedValue)(cacheKey, {
        ttlMs: 45000,
        staleMs: 75000,
        loader: async () => {
            const registerArgs = [
                ...
                const payload = await new Promise((resolve, reject) => {
                    ...
                    const timeoutId = setTimeout(() => {
                        ...
                        if (latest.length > 0)
                            ...
                            reject(new Error('Portal Pasazera GPS timeout'));
                    }, GPS_TIMEOUT_MS);
                    connection.on('TrainStatus', (...args) => {
                        if (settled)
                            return;
                        const points = Array.isArray(args[1]) ? args[1]
                            : (Array.isArray(args[0]) && Array.isArray(args[0][1]))
                                ? args[0][1]
                                : [];
...
function gpsVehicleId(point) {
    const numberKey = canonicalTrainNumber(point.trainNumber) || 'unknown';
    const latKey = Math.round(point.lat * 10000);
    const lngKey = Math.round(point.lng * 10000);
    return `pkp_intercity_gps_${numberKey}_${latKey}_${lngKey}`;
}
function trainNumberFromVehicleId(vehicleId) {
    ...
    if (parts.length >= 1 && /^\d+$/.test(parts[0]))
        return normalizeTrainNumber(parts[0]);
    return '';
}
function mapGpsPointToVehicle(point) {
...
async function loadStationsLookup() {
    return (0, cache_1.getCachedValue)('pkp_intercity:stations', {
        ttlMs: 24 * 60 * 60 * 1000,
        staleMs: 24 * 60 * 60 * 1000,
        loader: async () => {
            const lookup = {};
            for (let page = 1; page <= 40; page += 1) {
                const payload = await fetchJson('/api/v1/dictionaries/stations', { page, pageSize: 1000 });
...
async function loadOperations() {
    return (0, cache_1.getCachedValue)('pkp_intercity:operations', {
        ttlMs: 45000,
        staleMs: 90000,
        loader: async () => {
            const operations = [];
            ...
            for (let page = 1; page <= 20; page += 1) {
                const payload = await fetchJson('/api/v1/operations', {
                    carriersInclude: 'IC',
                    fullRoutes: true,
                    withPlanned: true,
                    pageSize,
...
async function loadRouteMetadata(scheduleId, orderId) {
    if (!scheduleId || !orderId)
        return null;
    const cacheKey = `pkp_intercity:route_meta:${scheduleId}:${orderId}`;
    const { value } = await (0, cache_1.getCachedValue)(cacheKey, {
        ttlMs: 12 * 60 * 60 * 1000,
        staleMs: 36 * 60 * 60 * 1000,
        loader: async () => {
            const payload = await fetchJson(`/api/v1/schedules/route/${encodeURIComponent(scheduleId)}/${encodeURIComponent(orderId)}`);
            if (!isRecord(payload))
                return null;
            ...
            const stationCategory = stations
                ...
                .find(Boolean);
            const trainNumber = normalizeTrainNumber(readString(payload, ['nationalNumber', 'commercialTrainNumber', 'trainNumber', 'number']) || stationNumber);
            const category = normalizeTrainCategory(readString(payload, ['commercialCategorySymbol', 'commercialCategory', 'category']) || stationCategory || 'IC');
...
function mapOperationToVehicle(operation, stations, now, gpsPoint, metadata) {
...
async function loadTrainDetails(vehicleId, stations, now) {
    ...
    const [payload, metadata, gpsSnapshot] = await Promise.all([
        fetchJson(`/api/v1/operations/train/${encodeURIComponent(scheduleId)}/${encodeURIComponent(orderId)}/${encodeURIComponent(operatingDate)}`),
        loadRouteMetadata(scheduleId, orderId).catch(() => null),
        fetchPortalGpsSnapshot(null).catch(() => ({ value: [], cache: 'miss' })),
    ]);
    ...
    const targetGps = gpsSnapshot.value.find((point) => canonicalTrainNumber(point.trainNumber) === canonicalTrainNumber(fallbackNumber));
    if (!targetGps)
        return null;
    return mapOperationToVehicle(operation, stations, now, targetGps, metadata);
}
exports.pkpIntercityProvider = {
    id: 'pkp_intercity',
    operatorName: OPERATOR_NAME,
    implemented: true,
    async getVehicles(options) {
        const gpsResult = await fetchPortalGpsSnapshot(options.bbox)
            .catch(() => ({ value: [], cache: 'miss' }));
    ...
    async getVehicleDetails(vehicleId) {
        const now = Date.now();
        const { value: stations } = await loadStationsLookup().catch(() => ({ value: {}, cache: 'miss' }));
        ...
        const [{ value: operations }, { value: gpsPoints }] = await Promise.all([
            loadOperations().catch(() => ({ value: [], cache: 'miss' })),
            fetchPortalGpsSnapshot(null).catch(() => ({ value: [], cache: 'miss' })),
        ]);
        const gpsByNumber = new Map();
        for (const point of gpsPoints) {
            const key = canonicalTrainNumber(point.trainNumber);
            if (key && !gpsByNumber.has(key))
                gpsByNumber.set(key, point);
        }
        if (!targetNumber) {
        ...
        for (const operation of operations) {
            if (metadataLookups >= METADATA_LOOKUP_LIMIT)
                break;
            const ids = idsFromOperation(operation);
            ...
            if (!metadata?.trainNumber)
                continue;
            if (canonicalTrainNumber(metadata.trainNumber) !== targetNumber)
                continue;
            const gpsPoint = gpsByNumber.get(targetNumber);
            if (!gpsPoint)
                return null;
